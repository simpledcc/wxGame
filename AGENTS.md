...
当前快照：H4 的加载/回退、页面绑定、按钮皮肤、Creator 元数据和微信 `home_common` 分包均有效；`assets/bundles/home_common` 已是高清运行结果并与 `art-source/home-v1/optimized` 的 18 张源图逐项同哈希。H8.4-H8.5 页面功能与布局未改，本次没有修改房间、玩法或页面交互。微信构建为 147 个文件、`9,970,161` 字节，主包 `4,121,077 / 4,194,304` 字节，`home_common` 分包 `3,472,932` 字节。
