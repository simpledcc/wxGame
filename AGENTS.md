...
当前工作流：A 线，首页和游戏准备前界面；H5 布局、H6 逻辑精简与 H7 赛前选项清理均已冻结。

当前目标：H7 赛前机器人/时长设置清理已完成；下一项是两台真实手机验证创建、加入、准备和开始流程，本阶段明确不进行 H4 美术接入或游戏内部规则改造。
...
当前快照：H5、H6 与 H7 已完成并冻结。赛前不再保存或展示总时长，不接受机器人难度，也不在创建房间请求中主动发送这两项；准备大厅仅显示玩法和两名玩家的准备状态。创建、加入和大厅仍只构造当前状态需要的节点，会话建立后入口表单立即销毁。游戏内部倒计时、机器人兼容逻辑、底层房间服务、云协议、玩法 Bundle、后台轮询、邀请、准备和开始能力保持不变。H4 继续延期，不得创建或导入美术资源。
