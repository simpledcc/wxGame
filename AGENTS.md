...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成一百三十一轮源码精修；第一百三十一轮把首页副标题文字框由 34px 收紧为实际 24px 行高，紧凑 40px 丝带获得上下各 8px、长屏 48px 丝带获得上下各 12px，左右继续各 17px，文案与外部位置不变。完整 `npm run verify` 和 `npm run build:wechat:dry-run` 均已通过。下一项唯一行动是审查首页创建、加入及双列辅助按钮中正式图标与两层文字组的纵向中心关系；在 Creator 3.8.8/微信开发者工具逐页视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 仅维护赛前展示与状态反馈，一百三十一轮详细证据见 `COCOS_PRE_GAME_FOUNDATION_PROGRESS.md` 和 `COCOS_PRE_GAME_PAGES_DESIGN.md`。当前源码回归覆盖全部主要赛前路由、最短高度、真实数据、按钮/卡片/文字层次、正式资源回退及交互状态；核心源码为 `1,519,947 / 1,520,000` 字节。上一版实际微信构建仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
