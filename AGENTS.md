...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成三轮源码精修：前两轮完成正式页头/卡片层次、按压/禁用/选中状态、学习进度、房间徽标、输入聚焦和战绩筛选；第三轮让词库解锁操作根据真实可解锁状态显示“解锁所选/无需解锁”，并让练习错词按钮在加入后保持选中并显示“已在错题库”。完整 `npm run verify` 与 `build:wechat:dry-run` 已通过。下一项唯一行动是在 Creator 3.8.8/微信开发者工具逐页检查本轮效果并根据截图继续细调；在视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 只调整共用赛前 UI 组件及 Bank、Study、Room、History 的展示状态，不修改房间协议、玩法 Bundle、云协议、AppID 或任何游戏内页面。源码回归已覆盖按钮按下/禁用/选中状态、卡片装饰、正式页头图标、学习进度、玩家准备徽标、输入聚焦、词库解锁、错词标记、战绩筛选和完整路由点击。核心源码为 `1,519,994 / 1,520,000` 字节，后续源码修改必须先继续收敛重复实现。上一版实际微信构建数字仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
