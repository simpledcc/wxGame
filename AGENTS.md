...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成一百三十八轮源码精修；第一百三十八轮将词库页底部同级的“解锁所选/确定选择”改为 `256x80/27px` 条件解锁和 `288x96/31px` 最终确认，图标形成 `44.8/53.76px` 递进。两者底边对齐、间隔 16px、左右外边距对称，最短屏均保留 8px 底部安全区；词库数据、金币解锁、选择状态、分页和返回逻辑不变。完整 `npm run verify` 和 `npm run build:wechat:dry-run` 均已通过。下一项唯一行动是审查房间大厅“开始游戏”相对准备、复制和邀请控制的视觉层级及禁用/可开始状态；不执行双人真机开始验证，在 Creator 3.8.8/微信开发者工具逐页视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 仅维护赛前展示与状态反馈，一百三十八轮详细证据见 `COCOS_PRE_GAME_FOUNDATION_PROGRESS.md` 和 `COCOS_PRE_GAME_PAGES_DESIGN.md`。当前源码回归覆盖全部主要赛前路由、最短高度、真实数据、按钮/卡片/文字层次、正式资源回退及交互状态；核心源码为 `1,519,962 / 1,520,000` 字节。上一版实际微信构建仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
