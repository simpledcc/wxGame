...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成一百四十三轮源码精修；第一百四十三轮保持帮助页卡片、六组真实规则和全部坐标不变，将编号/标题/正文由 `16/18/14px` 提升为 `18/20/15px`。最短屏每组仍保持 8px 标题/正文和至少 16px 组间距，长屏保留更宽松节奏；规则文案、返回玩法目录和路由行为不变。完整 `npm run verify` 和 `npm run build:wechat:dry-run` 均已通过。下一项唯一行动是进行一次跨页面主命令一致性审计，检查创建、加入、练习、词库、大厅、目录、反馈和战绩是否仍存在主/辅操作同级残余，并只修正有证据的问题；在 Creator 3.8.8/微信开发者工具逐页视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 仅维护赛前展示与状态反馈，一百四十三轮详细证据见 `COCOS_PRE_GAME_FOUNDATION_PROGRESS.md` 和 `COCOS_PRE_GAME_PAGES_DESIGN.md`。当前源码回归覆盖全部主要赛前路由、最短高度、真实数据、按钮/卡片/文字层次、正式资源回退及交互状态；核心源码为 `1,519,958 / 1,520,000` 字节，后续源码视觉调整仍不得抬高门槛。上一版实际微信构建仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
