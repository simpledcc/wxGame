...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成六轮源码精修：前五轮完成共用层次、按钮状态、学习/房间/词库反馈、输入计数与实时可用性；第六轮让房间玩家卡只由徽标表达准备状态，并为首页玩家/设置弹层增加身份徽标、正式设置图标和跟随真实静音状态的音效选中态。完整 `npm run verify` 与 `npm run build:wechat:dry-run` 已通过。下一项唯一行动是在 Creator 3.8.8/微信开发者工具逐页检查本轮效果并根据截图继续细调；在视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 只调整共用赛前 UI 组件及 Home、Bank、Study、Room、History、Feedback 的展示状态，不修改房间协议、玩法 Bundle、云协议、AppID 或任何游戏内页面。源码回归已覆盖按钮状态、卡片层次、正式图标、弹层状态、学习进度、房间徽标、输入聚焦/计数/实时校验、词库解锁、错词标记、战绩筛选和完整路由点击。核心源码为 `1,519,796 / 1,520,000` 字节，后续源码修改必须先继续收敛重复实现。上一版实际微信构建数字仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
