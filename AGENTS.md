...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成一百四十轮源码精修；第一百四十轮在不改变目录行横向 `8/8/8/9px` 边界和 80px 触控目标的前提下，将唯一可用“立即体验”视觉由 `126x62/17px` 提升为 `126x70/18px`，七个“筹备中”保持 `126x62/17px`、禁用视觉和不可点击状态。目录路由、模式可用性和未完成玩法均不变。完整 `npm run verify` 和 `npm run build:wechat:dry-run` 均已通过。下一项唯一行动是在不提高源码预算门槛的前提下先回收必要空间，再审查反馈页“提交反馈”相对状态带和隐私入口的视觉层级；在 Creator 3.8.8/微信开发者工具逐页视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 仅维护赛前展示与状态反馈，一百四十轮详细证据见 `COCOS_PRE_GAME_FOUNDATION_PROGRESS.md` 和 `COCOS_PRE_GAME_PAGES_DESIGN.md`。当前源码回归覆盖全部主要赛前路由、最短高度、真实数据、按钮/卡片/文字层次、正式资源回退及交互状态；核心源码为 `1,519,979 / 1,520,000` 字节，后续源码视觉调整必须先回收空间，不得抬高门槛。上一版实际微信构建仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
