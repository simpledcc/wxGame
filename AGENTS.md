...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成一百四十二轮源码精修；第一百四十二轮将战绩记录行“详情”可见区域由 `92x64` 提升为 `92x70`，实际触控继续为 80px、图标/标题继续为 `24/24px`；最短 86px 记录行上下各保留 8px，长屏 102px 行各保留 16px。按钮保持无位图皮肤的浅色紧凑处理，避免 70px 皮肤无法覆盖完整 80px 点击区。筛选选中态、摘要、真实记录、空状态、分页和详情弹层逻辑均不变。完整 `npm run verify` 和 `npm run build:wechat:dry-run` 均已通过。下一项唯一行动是审查帮助页六条规则的编号、标题、正文和卡片阅读层级；在 Creator 3.8.8/微信开发者工具逐页视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 仅维护赛前展示与状态反馈，一百四十二轮详细证据见 `COCOS_PRE_GAME_FOUNDATION_PROGRESS.md` 和 `COCOS_PRE_GAME_PAGES_DESIGN.md`。当前源码回归覆盖全部主要赛前路由、最短高度、真实数据、按钮/卡片/文字层次、正式资源回退及交互状态；核心源码为 `1,519,958 / 1,520,000` 字节，后续源码视觉调整仍不得抬高门槛。上一版实际微信构建仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
