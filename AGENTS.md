...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成一百三十七轮源码精修；第一百三十七轮将赛前练习页最终“下一个”由与辅助控制相同的 `500x72/24px` 提升为 `500x80/27px`，图标同步由 `40.32px` 提升为 `44.8px`。中文显示控制和最终命令上移 4px 后，最短屏仍形成释义/中文、中文/下一个各 8px 及底部 10px 的完整节奏；释义、错词、中文显示、进度、词库返回和学习状态不变。完整 `npm run verify` 和 `npm run build:wechat:dry-run` 均已通过。下一项唯一行动是审查词库页“确定选择”相对条件式“解锁所选”和分页控制的视觉层级；在 Creator 3.8.8/微信开发者工具逐页视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 仅维护赛前展示与状态反馈，一百三十七轮详细证据见 `COCOS_PRE_GAME_FOUNDATION_PROGRESS.md` 和 `COCOS_PRE_GAME_PAGES_DESIGN.md`。当前源码回归覆盖全部主要赛前路由、最短高度、真实数据、按钮/卡片/文字层次、正式资源回退及交互状态；核心源码为 `1,519,962 / 1,520,000` 字节。上一版实际微信构建仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
