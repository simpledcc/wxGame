...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成一百三十三轮源码精修；第一百三十三轮把有副标题且高度至少 100px 的共享操作标题/副标题整体上移 2px，并把正式图标恢复到 `y=0`，使 118/136/146px 首页长按钮完整内容块的上下边界差由 3.6/3.2/5.2px 收敛到 0.4/0.8/1.2px，同时保持图标与两层文字组中心偏差不超过 1px，紧凑按钮不变。完整 `npm run verify` 和 `npm run build:wechat:dry-run` 均已通过。下一项唯一行动是审查首页“创建房间/加入房间”相对“赛前练习/选择词库”的字号、图标和色彩主次层级；在 Creator 3.8.8/微信开发者工具逐页视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 仅维护赛前展示与状态反馈，一百三十三轮详细证据见 `COCOS_PRE_GAME_FOUNDATION_PROGRESS.md` 和 `COCOS_PRE_GAME_PAGES_DESIGN.md`。当前源码回归覆盖全部主要赛前路由、最短高度、真实数据、按钮/卡片/文字层次、正式资源回退及交互状态；核心源码为 `1,519,962 / 1,520,000` 字节。上一版实际微信构建仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
