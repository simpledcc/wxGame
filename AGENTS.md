...
当前工作流：A 线，首页和游戏准备前界面；H5 布局与 H6 逻辑精简均已冻结。

当前目标：H6 赛前逻辑精简已完成；下一项是两台真实手机验证创建、加入、准备和开始流程，本阶段明确不进行 H4 美术接入。
...
当前快照：H5 与 H6 已完成并冻结。模式目录不再保留旧双合作控制器入口和重复词库状态，房间页面不再保留隐藏机器人/手动刷新控制器；创建、加入和大厅只构造当前状态需要的节点，会话建立后入口表单立即销毁。完整回归、Creator 微信构建、包体检查和开发者工具启动均通过；底层房间服务、云协议、玩法 Bundle、后台轮询、邀请、准备和开始能力保持不变。H4 继续延期，不得创建或导入美术资源。
...
开发时遵循现有架构和测试模式。先读取相关实现，再做最小范围修改；不得为了 H6 重写 Router、房间协议或业务服务，也不得删除仍被可见页面、邀请恢复或底层兼容测试使用的能力。
