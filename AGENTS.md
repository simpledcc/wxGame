...
当前目标：继续美化全部游戏进入前页面的按钮、卡片、文字和状态反馈。H8.7 已在无 Creator 电脑完成三十七轮源码精修：前三十六轮完成状态反馈、正式图标、动态边界、主要页面最短高度、按钮/页头/卡片、词库信息、准备大厅、共享强调卡、学习页及首页工具层次；第三十七轮把创建配置“更换”从纯文字按钮升级为正式词库图标和纯标题结构，保持原 `118x80` 点击区、长词库列及换库返回逻辑。完整 `npm run verify` 与 `npm run build:wechat:dry-run` 已通过。下一项唯一行动是在 Creator 3.8.8/微信开发者工具逐页检查本轮效果并根据截图继续细调；在视觉证据完成前 H8.7 保持 `IN_PROGRESS`。
...
当前快照：H4 高清运行资源、Creator 元数据和 `home_common` 分包保持不变；H8.7 只调整共用赛前 UI 组件及 Home、Bank、Study、CoopSelect、Room、History、Feedback、Help 的展示状态，不修改房间协议、玩法 Bundle、云协议、AppID 或任何游戏内页面。源码回归已覆盖按钮状态、动作文字间距、共享强调卡左侧竖条及图标/文字列、玩法标题/说明间距、首页头像/真实系统名/安全身份/金币横向层次、金币图标/动态数字/加号圆徽标/单点击区、首页词库标题/名称/图标/徽标层次、创建配置模式标题/说明及词库标签/正文图标/动态名称/正式换库图标标题层次、准备大厅玩家卡标签/单头像/状态徽标/玩家名层次、房间码/复制图标标题/邀请正式图标标题三列及词库/状态卡内边距、学习卡进度标题/状态/进度条/单词/释义/图标导航双按钮/底部内边距、页头标题/分隔线/图标/副标题层次、短卡片紧凑标题标签及正文图标/文字间距、大卡片原规格、卡片层次、正式图标、弹层状态及元素间距、全部主要赛前页面最短高度边界、玩法/房间徽标及文字边界、房间准备操作图标与选中态、唯一离开入口、动态文字列、同步状态、玩法说明、表单状态、输入校验、词库解锁、错词标记、战绩筛选和完整路由点击。核心源码为 `1,519,853 / 1,520,000` 字节，后续源码修改必须先收敛重复实现再扩展。上一版实际微信构建数字仍只作为 H8.6 基线，不作为 H8.7 新画面的外部证据。
