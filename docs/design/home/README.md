...
当前完成状态是“首页结构、真实数据、交互和可替换美术接口完成”；正式竖屏背景、Logo、角色和图标美术仍需单独完成并接入。
...
创建/加入房间、词库、练习、玩法、战绩、设置、隐私和反馈均已绑定现有 route、Store 或平台服务。首页不伪造等级、金币、玩家资料和战绩。
...
- 13 个稳定资源槽。
- 缺图时仍可操作的程序化/文字 fallback。
- 背景 cover、前景 contain 的等比例适配。
- 图标替换不影响按钮命中、route 和 Store 的契约。

当前房屋、双人、书本、词库、手柄、奖杯、齿轮、隐私和反馈仍显示为“房、友、练、词、玩、绩、设、隐、言”等 fallback，而不是参考图中的正式卡通图标。
...
| 竖屏村庄背景 | 通用横向草地/海岛背景 cover | 天空、树叶、村庄、道路 | 未完成 |
| 立体 Logo | 分层文字 fallback | 独立透明卡通 Logo | 未完成 |
| 角色装饰 | 文字/图形占位 | 兔子或同风格角色 | 未完成 |
| 功能图标 | 单字 fallback | 统一卡通图标 | 未完成 |
| 按钮质感 | 程序化圆角、描边、阴影 | 更丰富的九宫格和高光质感 | 基础完成，高保真未完成 |
...
2. 设计透明“词斗乐园”Logo。
3. 设计安全头像和首页角色装饰。
4. 设计创建、加入、练习、词库、玩法、战绩、设置、隐私和反馈图标。
