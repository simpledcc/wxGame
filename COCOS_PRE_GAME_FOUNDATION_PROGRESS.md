...
### H8.7 ninety-sixth source checkpoint

- Moved the lobby room-code value, Copy and Invite columns together from `y=-6` to `y=-4`; the three columns remain on one horizontal axis.
- Tightened only the room-code value box from 48px to 44px, preserving its top edge and 30px font while both action nodes keep their original 80px touch height. Copy and Invite now clear the card bottom frame by eight pixels instead of six.
- Runtime coverage freezes the eight-pixel action boundary. Room-code copy, invitation handling, room data and protocols remain unchanged.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,983 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 ninety-seventh source checkpoint

- Tightened both lobby player cards from 220px to 218px while preserving their `y=2` center axis and every avatar, ready/waiting badge and real-name coordinate.
- The current-Bank/player-card and player-card/current-status boundaries now both keep eight pixels instead of seven. No content was moved and the cards retain ample internal and bottom safety area.
- Runtime coverage freezes the 218px height and requires the complete minimum-height lobby chain to retain at least eight pixels. Player identity, readiness, polling and start eligibility remain unchanged.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,983 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 ninety-eighth source checkpoint

- Tightened all four Bank rows from 96px to 88px. At the 360px-wide target gate each row still provides approximately 49.5px of touch height, while its title/subtitle boxes scale from `42/32px` to `38/28px` and retain the existing `6/8/8px` top/inter-row/bottom rhythm.
- Shifted the Bank status card and first-row axes down four pixels and the pager axis down two pixels. The minimum-height header/status/four-row/pager chain now keeps at least eight pixels throughout instead of the former four-pixel header/status and status/row plus six-pixel row/pager bottlenecks.
- Runtime coverage freezes the 88px row height, six-pixel title inset, eight-pixel title/subtitle and subtitle/bottom boundaries, and the full minimum Bank chain. Real Bank names, word counts, badges, selection, unlock, paging and return behavior remain unchanged.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,983 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 ninety-ninth source checkpoint

- Rebalanced Home's compact subtitle, current-Bank, Create, Join, learning and extension axes from offsets `67/131/223/323/415/499` to `67/135/229/329/423/511`. The complete minimum-height main-action chain now keeps at least eight pixels instead of repeated four-pixel boundaries and still leaves nine pixels before the footer actions.
- Tightened compact Create and Join from 96px to 92px. At the 360px-wide target gate both still provide approximately 51.8px touch height and remain visibly larger than the unchanged 80px secondary actions.
- Long Home also moves the Bank, learning and extension axes to `174/587/714`, raising its remaining subtitle/Bank and lower-row boundaries to at least eight pixels. Formal skins, title/subtitle geometry, character calculation, privacy/feedback footer and all route handlers remain unchanged.
- Runtime coverage freezes both long and minimum eight-pixel chains plus the compact 92px primary height. Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,983 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundredth source checkpoint

- Changed all eight catalog cards from 80px visible height on 84px centers to 78px visible height on 86px centers, while restoring each row layout node to 80px. Visible card gaps rise from four to eight pixels without reducing target-device action touch areas.
- Tightened each formal/fallback mode icon and action visual from 64px to 62px, retaining eight pixels inside the visible card. Titles and subtitles move to `y=18/-18`, producing a balanced visible `7/8/7px` top/inter-row/bottom rhythm; player badges move to `y=-14` for an eight-pixel bottom boundary.
- Runtime coverage freezes the 78px outer card through its 66px inner border and 42px accent rail, 62px action visual, 80px action touch height, balanced copy and visual eight-pixel row gaps. Category colors, formal icons, Help entry, first mode route and all seven disabled actions remain unchanged.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source at `1,519,985 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-first source checkpoint

- Tightened Home Privacy and Feedback from 84px to 80px and moved their center axis from `safeBottom+46` to `safeBottom+48`. Their top edge remains unchanged, so the nine-pixel extension/footer boundary from the previous pass is preserved.
- Both actions now keep an exact eight-pixel bottom safe-area boundary instead of four pixels in long and minimum layouts. At the 360px-wide target gate each still provides approximately 45px touch height.
- Runtime coverage freezes both heights and bottom clearances across long/minimum Home. Formal button skins, Privacy pending/disabled/restore behavior, Feedback navigation, labels and horizontal layout remain unchanged.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,985 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-second source checkpoint

- Added a shared 80px minimum touch height to `PreGameUi.actionButton` without changing the requested visible geometry. The existing History detail action therefore keeps its 64px inset visual through the shared path instead of a page-specific node override.
- Tightened Study Bank, Reveal, Meaning and Next visuals from 80px to 72px while retaining 80px click nodes. The complete long and minimum Study page chain now keeps at least eight visible pixels from the header through Bank, card and lower actions to the safe-area bottom.
- Runtime coverage freezes both visual and touch heights, all five page boundaries, the final safe-area clearance and the unchanged Study interactions. Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source at `1,519,995 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No Study data/state, History behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-third source checkpoint

- Tightened the Study Mark Wrong visual from 80px to 72px while retaining its shared 80px target-device touch node. It now matches the adjacent `252x72px` Reveal action instead of producing a visibly uneven pair on the same row.
...
### H8.7 one-hundred-and-sixth source checkpoint

- Shifted the shared create/join content axis and the preparation-lobby content axis down three pixels. The create configuration's Header/Selected Mode boundary and the active lobby's Header/Room Code boundary rise from five to exactly eight pixels in both long and minimum layouts; the Join card keeps its existing wider clearance.
- Added one shared room Header/first-card geometry contract across all three entry states and both viewport classes. Existing create/lobby internal eight-pixel chains and minimum safe-bottom checks continue to pass, so the adjustment does not shrink card content or button touch targets.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No card/button size, room data/state/action, protocol, bitmap, importer metadata, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-seventh source checkpoint

- Moved all four Bank-row title boxes from `y=19` to `y=18` and tightened their height from 38px to 36px. Each unchanged 88px card now presents exact `8/8/8px` title-top, title/subtitle and subtitle-bottom spacing instead of the former `6/8/8px` imbalance.
- The 27px title retains its 32px line height inside the 36px box; the 15px subtitle, real Bank name/count, SHRINK behavior, selection badge, card size and target-device touch height remain unchanged. Long and minimum runtime tests freeze the title box plus all three boundaries.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No Bank data/selection/unlock behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-eighth source checkpoint

- Tightened all four History record title/meta boxes from `y=22,h=32` and `y=-20,h=34` to `y=17,h=28` and `y=-18,h=26`. At the minimum 86px row height the text block changes from `5/9/6px` to centered `12/8/12px` top/inter-row/bottom spacing; the long 102px row uses `20/8/20px`.
- Existing 18px title and 13px metadata fonts retain their 22px and 17px line heights inside the new boxes. Real record copy, score/detail columns, 64px detail visual, 80px detail touch node, row/card size, filtering and paging remain unchanged. Shared runtime coverage freezes all four rows in both viewport classes.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No History data/detail/filter behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-ninth source checkpoint

- Moved all five History filter labels from `y=4` to `y=7` and their six-pixel selected markers from `y=-21` to `y=-16`. The unchanged 56px visible button now uses exact `9/8/9px` top/copy-marker/bottom spacing instead of leaving only four pixels below the marker.
- Filter layout and click nodes remain 80px high for the target-device gate. All/Pk/Coop/Spell selection, marker visibility, real record filtering and the disabled Other action remain unchanged; runtime coverage freezes the exact internal geometry while continuing to exercise selection transitions.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No History data/filter/paging behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-tenth source checkpoint

- Shifted all four History record title/meta columns from `x=-82,w=286` to `x=-64,w=258`. Their former 24px overlap with the 58px formal History icon is removed; each row now has exact eight-pixel icon/copy and copy/score boundaries.
- Both text rows retain the previous centered vertical geometry, 18/13px fonts and SHRINK behavior inside the narrower safe column. Real mode/time copy, score column, 64px Detail visual, 80px Detail touch node, row size, filtering and paging remain unchanged. Shared runtime coverage freezes both boundaries on all four rows in long and minimum layouts.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No History data/detail/filter behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-eleventh source checkpoint

- Moved all six Help rule title/body axes from `y+20/y-18` to `y+22/y-16`. Each combined text block moves up two pixels so its outer top/bottom edges are symmetric around the numbered badge instead of centering two pixels below it.
- The 24/36px text boxes, 18/14px fonts, exact eight-pixel title/body gap, inter-rule spacing, horizontal badge/copy boundary, real six-rule content, Help card and return route remain unchanged. Shared long/minimum runtime coverage freezes all six badge/text centers.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No Help content/route behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-twelfth source checkpoint

- Narrowed the Feedback status label from 500px to 468px inside the unchanged 500x34px status band. The status copy now keeps exact 16px left/right insets while retaining its existing four-pixel top/bottom insets.
- Empty, validation, pending, success and server-failure states keep their current copy, 18px font, color updates and submission lifecycle. Shared runtime coverage freezes both horizontal and vertical boundaries without changing the form, native inputs, submit action, privacy action or route.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No Feedback behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-thirteenth source checkpoint

- Shifted and widened the Help summary copy from `x=52,w=360` to `x=40,w=428` beside the unchanged `x=-218,w=72` catalog icon. The former 54px gap becomes an exact eight-pixel icon/copy boundary.
- The icon left edge and copy right edge now retain equal 26px card insets. Summary copy, 17px font, vertical axis, formal icon, six real Help rules, card geometry and return route remain unchanged; shared long/minimum runtime coverage freezes both horizontal boundaries.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No Help content/route behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-fourteenth source checkpoint

- Widened Feedback prompt copy from 424px to 448px beside its unchanged 48px formal icon. The row changes from a 20px icon/copy gap with 28/40px outer insets to an exact eight-pixel gap and equal 28px card insets.
...
### H8.7 one-hundred-and-sixteenth source checkpoint

- Shifted and widened Create Bank name copy from `x=-18,w=300` to `x=-17,w=304`. The unchanged 66px formal icon, real SHRINK-enabled name and unchanged 118px Change action now form exact eight-pixel icon/name and name/action boundaries instead of the former 9/11px chain.
- The compact Change action's existing 24px icon/title layout is now explicitly frozen at an eight-pixel boundary. Card geometry, fonts, action visual/touch size, real Bank selection, picker return and auto-ready persistence remain unchanged; shared long/minimum runtime coverage freezes the complete three-column chain.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No room/Bank behavior, bitmap, importer metadata, protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-seventeenth source checkpoint

- Shifted and widened Room Lobby status copy from `x=42,w=420` to `x=18,w=436` beside the unchanged overlapping ready/attention indicators at `x=-222,w=28`. Their former 40px indicator/copy gap and 24/8px outer insets become an exact eight-pixel gap with equal 24px card insets.
- Copy, 17px font, vertical bottom alignment, waiting/attention/ready visibility, quiet polling, Ready selection and Start highlight behavior remain unchanged. Shared long/minimum runtime coverage freezes both indicator variants and continues to exercise the complete room-state transitions.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source unchanged at `1,519,895 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No room protocol/behavior, bitmap, importer metadata, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-eighteenth source checkpoint

- Shifted/widened Room Code copy from `x=-88,w=216` to `x=-85,w=222` and moved the unchanged 112px Copy action from `x=84` to `x=90`; the 112px Invite action remains at `x=210`. Code/Copy, Copy/Invite and Invite/card-right boundaries now form a continuous `8/8/8px` chain instead of `8/14/8px`, and both compact actions freeze their internal icon/title gaps at eight pixels.
- Centered and widened Lobby Bank copy from `x=18,w=430` to `x=0,w=472`, replacing 63/27px outer insets with equal 24px card insets. Copy, invite, real Bank text, fonts, touch geometry and room behavior remain unchanged; shared long/minimum runtime coverage freezes both utility cards.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source at `1,519,894 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No room protocol/behavior, bitmap, importer metadata, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-nineteenth source checkpoint

- Shifted and widened all eight catalog title/subtitle columns from `x=-65,w=236` to `x=-67,w=240` beside the unchanged `x=-226,w=62` formal icons. The icon/copy gap tightens from 12px to an exact eight pixels while the copy right edge remains fixed.
- Every row now forms a continuous `8/8/8px` icon/copy, copy/badge and badge/action chain. Copy, fonts, vertical rhythm, action geometry, the first real route and the other seven disabled states remain unchanged; shared long/minimum runtime coverage freezes both text rows and the complete horizontal chain across all eight entries.
...
### H8.7 one-hundred-and-twentieth source checkpoint

- Shifted all eight catalog formal icons from `x=-226` to `x=-230` and expanded their title/subtitle columns from `x=-67,w=240` to `x=-69,w=244`. The copy right edge, status badges and action buttons remain fixed, tightening the accent-rail/icon boundary from 12px to an exact eight pixels.
- Every row now forms a continuous `8/8/8/8px` accent-rail/icon, icon/copy, copy/badge and badge/action chain. Copy, fonts, vertical rhythm, card/action geometry, the first real route and the other seven disabled states remain unchanged; shared long/minimum runtime coverage freezes the complete horizontal chain across all eight entries.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source at `1,519,894 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No route behavior, bitmap, importer metadata, room protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-twenty-first source checkpoint

- Shifted and widened Study Bank title/subtitle copy from `x=-20,w=350` to `x=-31.84,w=351.68` and moved the unchanged 104px Change badge from `x=220` to `x=204`. The unchanged 40.32px formal icon, both copy rows and the badge now form exact eight-pixel internal boundaries with equal 24px outer insets.
- Real Bank text, fonts, SHRINK behavior, visual/touch geometry, the whole strip's single Button and Bank return flow remain unchanged. Shared long/minimum runtime coverage freezes both rows, all four horizontal boundaries and the absence of a nested badge Button.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source at `1,519,900 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No route behavior, bitmap, importer metadata, room protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-twenty-second source checkpoint

- Shifted and widened all four Bank-row title/subtitle columns from `x=-32,w=300` to `x=-31.36,w=314.72` and moved their unchanged 112px state badges from `x=198` to `x=190`. The unchanged 49.28px formal icons, both copy rows and badges now form exact eight-pixel internal boundaries with equal 24px outer insets.
- Real Bank names and word counts, SHRINK behavior, selected/unlocked/locked feedback, 88px card/touch geometry, paging, unlock and confirm behavior remain unchanged. Shared long/minimum runtime coverage freezes both text rows and all four horizontal boundaries on every visible entry.
- Full `npm run verify` and `npm run build:wechat:dry-run` pass with core source at `1,519,912 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No route behavior, bitmap, importer metadata, room protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-twenty-third source checkpoint

- Made Home Bank copy responsive to its 80/92px action height. Compact title uses `x=-23.6,w=319.2` with caption `x=-113.2`; long title uses `x=-20.24,w=312.48` with caption `x=-106.48`. Moved the unchanged 92px Change badge from `x=204` to `x=190`.
...
### H8.7 one-hundred-and-twenty-seventh source checkpoint

- Expanded Home's Study/Bank, Catalog/History and Privacy/Feedback columns from 252px to 256px and moved their centers from `x=-134/134` to `x=-132/132`. Each pair's former 16px gutter becomes the shared eight-pixel rhythm while equal SafeArea outer insets remain 36px.
...
### H8.7 one-hundred-and-twenty-ninth source checkpoint

- Rebuilt Home's optional companion slot around the formal `512x768` resource ratio. The slot now uses exact `2:3` geometry and computes its horizontal center from the current height, preserving a 24px right SafeArea inset across long portrait ratios instead of using one fixed x-coordinate with a wider fallback box.
- The companion's reserved vertical inset grows from 8px to 16px above and below, and its visibility threshold rises from 136px to 152px so the 120px minimum decoration is never shown without full breathing room. `393x852` and `360x800` show a bounded responsive character; `640x960` still hides it and retains every functional footer action.
- Focused three-viewport runtime coverage, full `npm run verify` and `npm run build:wechat:dry-run` pass with core source at `1,519,938 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No copy, action, route, bitmap, importer metadata, room protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-thirtieth source checkpoint

- Replaced compact Home's `520x108` Logo slot with an exact `416x130` `16:5` slot matching the formal `1280x400` resource. The loaded Logo now renders at the slot's full 416px width instead of being height-limited to 345.6px, giving it 22px of visual precedence on each side of the unchanged 372px subtitle ribbon; the programmatic fallback and formal Sprite also share one boundary.
...
### H8.7 one-hundred-and-thirty-fourth source checkpoint

- Replaced compact Home's equal 92/92px Create/Join actions with a 96/88px pair. Both centers move two pixels downward, so the Bank/Create, Create/Join and Join/learning gaps remain exactly eight pixels and the action stack's top and bottom boundaries remain unchanged.
...
### H8.7 one-hundred-and-thirty-seventh source checkpoint

- Enlarged Study's final Next command from `500x72` to `500x80`. Its shared title tier rises from 24px to 27px and its formal practice icon grows from 40.32px to 44.8px, while Reveal, Mark-wrong and Meaning Toggle remain auxiliary `72px/24px` visuals.
...
### H8.7 one-hundred-and-fortieth source checkpoint

- Enlarged only the mode catalog's available “立即体验” visual from `126x62/17px` to `126x70/18px`; its actual Button target remains 80px high. The seven “筹备中” actions remain `126x62/17px`, disabled and non-interactable, so unfinished modes cannot acquire false affordance.
...
### H8.7 one-hundred-and-forty-second source checkpoint

- Enlarged each History record's compact Detail visual from `92x64` to `92x70`; its actual target remains 80px high, and the semantic history icon/title remain `24/24px`. At the true minimum viewport, 86px record rows now retain exact eight-pixel visual top/bottom insets; long 102px rows retain 16px.
...
### H8.7 one-hundred-and-forty-fourth source audit checkpoint

- Audited the final-command hierarchy across all active preparation paths. Runtime assertions now prove: Create `480x128/38px` over Auto-ready `390x80/27px`; Join `520x120/38px` over its `500x104` input; Study Next `500x80/27px` over `500x72/24px` Meaning Toggle; Bank Confirm `288x96/31px` over Unlock `256x80/27px`; Lobby Start `520x104/31px` over Ready `440x80/27px`; Catalog Experience `126x70/18px` over seven disabled `126x62/17px` actions; Feedback Submit `560x94/31px` over Privacy `520x80/27px`; and History Detail as a bounded `92x70/24px` row command.
- The audit found no remaining source-proven primary/secondary equality. Home's primary chain and same-level paired auxiliary actions already have separate responsive contracts; Help is informational and has no competing final command. No arbitrary geometry change was made after this finding.
- The unchanged source has passed full `npm run verify` and `npm run build:wechat:dry-run` at `1,519,958 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No bitmap, importer metadata, behavior, gameplay Bundle, cloud function, legacy client, protocol or AppID changed. H8.7 remains `IN_PROGRESS` because current Creator/WeChat screenshots do not yet exist; not `dev_done`.

### H8.7 one-hundred-and-forty-fifth source checkpoint

- Removed the History row's forced full locale date/time from the 258px metadata column. Each real record now displays `M月D日 · 词库名`, preserving both the scan-useful date and stored Bank label while avoiding seconds and time-of-day competing for the same single line. The Recent Summary card similarly keeps its mode first line and changes only the second line from a full date/time to the local date.
- Raised row metadata from 13px to 15px inside the unchanged 258x26 box. Titles, scores, Detail actions, row/card geometry, original timestamps, persisted labels, filtering, paging and detail behavior remain unchanged. Runtime coverage freezes the compact date prefix, 15px configured type and no-time Recent Summary.
- Focused runtime coverage, full `npm run verify` and `npm run build:wechat:dry-run` pass at `1,519,999 / 1,520,000`; formal art remains `imported` with 18 images and 23 metadata files. No bitmap, importer metadata, room/gameplay/cloud protocol, gameplay Bundle, cloud function, legacy client or AppID changed. Future source edits must first recover payload bytes and current Creator/WeChat visual evidence remains pending; not `dev_done`.

### H8.7 one-hundred-and-forty-sixth source checkpoint

- Raised the fixed Home player identity from 13px to 15px, the shared status-badge default from 14px to 15px, and multiline input counters from 13px to 15px. This covers the visible Home identity, room ready/waiting states, catalog player-count badges, player-modal identity, room-code count and Feedback count without changing any node, hit target or state behavior; explicit large `+` and Help number badges keep their existing larger sizes.
...
### H8.7 one-hundred-and-forty-eighth source checkpoint

- Strengthened semantic Accent Card rails without changing their horizontal contract. Width remains 4px, the outer-card inset remains one pixel, the inner-frame clearance remains one pixel and icon columns retain their eight-pixel boundary; top/bottom inset falls from 18px to 14px. The 78px catalog-card rail consequently grows from 42px to 50px, and all taller Join/Result/History/Feedback/Help cards use the same `card height - 28px` rule.
...
### H8.7 one-hundred-and-fiftieth source checkpoint

- Rebuilt the shared non-Home header as a compact semantic title panel. Bank, Study, Catalog, Create/Join/Lobby, History, Feedback and Help now supply their real action kind, so the header icon uses the existing formal resource/fallback instead of a stray leading Chinese character. Title, divider and subtitle retain fixed non-overlapping boxes.
- Replaced the Catalog's beige text rows and small trailing controls with eight full-width `548x80` semantic action buttons. `好友房间体验` remains the only enabled preparation entry; all future modes show a quiet `筹备中` badge and stay non-interactable. Home `玩法介绍` now opens the introduction page directly, while Catalog Help returns to Catalog through presentation-only Store state.
- Compacted Create/Lobby short-card tabs and aligned mode/Bank colors with their semantic actions. The Help page now contains eight concise gameplay introductions rather than duplicating the selectable Catalog. No gameplay implementation, room/cloud protocol or mode availability changed.
- Typecheck plus focused and full-route runtime tests pass. Creator 3.8.8 produced a real WeChat build with 147 files, `4,121,077 / 4,194,304` main-package bytes and `3,472,932` `home_common` bytes. The normalized source warning rises narrowly from `1.52` to `1.53 MB` because the previous baseline had only 13 bytes left; the real `4 MiB` package gate is unchanged. Current Creator/WeChat multi-page screenshots remain the final visual gate; not `dev_done`.
- The built project was then traversed in WeChat Developer Tools with formal `home_common` art on Home, gameplay selection, room setup and gameplay introduction. Header pseudo-characters are absent, all eight mode rows use complete semantic skins, room cards have compact title tabs and Help contains eight informational topics. An initial `loadSubpackage:fail module not found` was traced to Developer Tools compiling while Creator moved the Bundle from the temporary `assets/home_common` location to `subpackages/home_common`; one normal compile after the external build restored every formal asset and removed the subpackage errors. This is a build-preview cache procedure, not a runtime source or package-layout defect.
...
3. Creator import/rendering and WeChat simulator presentation now pass through the H8.7 one-hundred-and-fiftieth checkpoint on the iPhone 12/13 preset; remaining target-size captures, two-real-phone room acceptance, low-end performance and upload remain release QA.
...
Open the current branch with Cocos Creator 3.8.8 and WeChat Developer Tools, traverse every H8.7 pre-game page and capture the remaining target-size state evidence. After an external Creator build, run one normal Developer Tools compile before judging resources. Check the eight full-width catalog actions, same-axis Back/rules circles, semantic header icons, title/subtitle safety boundary, formal-skin normal/pressed/disabled transitions, Home player/coin/Bank hierarchy, compact and large section tabs, room setup cards, Study action chain, Help's eight numbered information rows, History filters/summaries and all native-input/state feedback. Two-device gameplay acceptance remains deferred until H8.7 is complete.
