...
- 2026-07-14 H4 cross-computer import automation: `npm run home-art:prepare` now performs an exact 18-file, hash-protected copy into the transient `assets/bundles/home_common/textures` import worktree and refuses unexpected files, changed files or an already imported directory. After Creator 3.8.8 generates metadata, `npm run home-art:verify-import` requires byte-identical approved images, Bundle name `home_common`, 23 Bundle/directory/image metadata files, unique top-level UUIDs and a generated SpriteFrame sub-resource for every image. `npm run test:home-art-import` exercises preparation, idempotence, tamper rejection and the metadata gate without Creator. The current computer still does not create or commit importer metadata; the designated Creator owner runs these commands and commits the imported tree plus generated metadata atomically.
