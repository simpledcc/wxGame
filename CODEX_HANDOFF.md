...
- 2026-07-14 H4 art-generation/code-binding checkpoint: four new source masters and 18 optimized import files are under `cocos-client/art-source/home-v1/`; the optimized payload is `332,877` bytes. `HomeArtManager` now deduplicates/caches/retries `home_common` loads, every existing visual slot binds by semantic key, sliced orange/blue/green/purple button skins preserve live Labels/actions, non-game routes prefer the shared learning-garden background, and build tooling requires `home_common` as a WeChat subpackage. This computer has no Creator, so no file was placed under `assets/` and no importer `.meta` was hand-written. Next action: one designated Creator 3.8.8 computer creates/imports `home_common`, commits images plus all generated metadata atomically, runs full verification/actual build/inspection, and captures Creator visual evidence. This is an in-progress checkpoint, not H4 `dev_done`.
