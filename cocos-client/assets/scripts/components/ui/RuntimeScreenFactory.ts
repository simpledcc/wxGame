...
export class RuntimeScreenFactory {
  ...
  private buildHome(parent: Node, ui: RuntimeUi): Node {
    ...
    home.actionButton(
      safe.node, "CreateRoomButton", "创建房间", "邀请好友，一起开始对战", "房", 0, logoY - fit(229, 302), 520, fit(92, 146),
      () => controller.openModeCatalog(), "create", "createRoom"
    );
    home.actionButton(
      safe.node, "JoinRoomButton", "加入房间", "输入房间码，快速加入好友对局", "友", 0, logoY - fit(329, 452), 520, fit(92, 136),
  ...
  private buildHelp(parent: Node, ui: RuntimeUi): Node {
    ...
    HELP_RULES.forEach(([title, copy, kind], index) => {
      ...
      home.label(helpCard, `HelpRule${index}Title`, title, 27, y + 20, 430, 24, 18, "homeText", 0);
      home.label(helpCard, `HelpRule${index}Body`, copy, 27, y - 18, 430, 36, 14, "homeTextMuted", 0);
    });
