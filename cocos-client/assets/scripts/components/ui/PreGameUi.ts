...
import { PreGameIconRenderer } from "./PreGameIconRenderer";
import { HOME_VISUAL_SLOT_KEYS, type HomeVisualSlotKey } from "./HomeVisualTypes";

export { HOME_VISUAL_SLOT_KEYS };
export type { HomeVisualSlotKey };
...
export class PreGameUi {
  private readonly icons: PreGameIconRenderer;

  constructor(readonly theme: ThemeManifest) {
    this.icons = new PreGameIconRenderer({
      node: (...args) => this.node(...args),
      label: (...args) => this.label(...args),
      color: (token) => this.color(token)
    });
  }
  ...
  playerStatusCard(parent: Node, name: string, title: string, x: number,
    ...
    const card=this.sectionCard(parent, `${name}Card`, title, x, 2, 264, 218, kind);
  ...
  actionButton(parent: Node, name: string, title: string, subtitle: string, fallbackIcon: string,
    ...
    const titleY=subtitle?(height>=120?26:height>=100?22:height>=96?20:height>=88?18:16):0;
  ...
  private drawProgrammaticIcon(
    ...
    return this.icons.drawIcon(parent, key, width, height, token);
  }

  private drawProgrammaticLogo(parent: Node, width: number, height: number): Node {
    return this.icons.drawLogo(parent, width, height);
  }
