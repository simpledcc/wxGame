...
- Current preparation Room UI exposes no robot or duration selector and uses normalized Player 1/Player 2 rows; gameplay-level robot compatibility remains outside the pre-game workstream.
...
- Commit `bfd2870` established the H6 baseline; the current H7 worktree also has successful Cocos Creator 3.8.8 WeChat builds and Developer Tools runs. The H7 package is `6,487,834` bytes total, with `4,120,918 / 4,194,304` bytes in the main package and `2,366,916` bytes in subpackages; future art must not consume the remaining main-package margin.
