...
| 4 - room flow | Create/join/copy/invite/ready/start, normalized player rows, unified pending UI and authoritative snapshots | Room service/flow/lifecycle/runtime tests cover success, replacement failure, accepted-join recovery, non-duplicated names and all visible-command busy locking; H8.1 requires two real humans and H8.2 keeps normal polling visually quiet while highlighting the ready start action | Implemented / two-device proof pending |
